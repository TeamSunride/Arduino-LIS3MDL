...
byte LIS3MDL::who_am_i() {
    return device->read_reg(LIS3MDL_REGISTER::WHO_AM_I);
}

uint8_t LIS3MDL::enable_temp_sensor(bool enable) {
    byte data = device->read_reg(LIS3MDL_REGISTER::CTRL_REG1);
    setBit(&data, 7, enable);
    return device->write_reg(LIS3MDL_REGISTER::CTRL_REG1, data);
}
...
uint8_t LIS3MDL::set_mag_ODR(LIS3MDL_OUTPUT_DATA_RATES odr) {
    ...
    switch (odr) {
        ...
        case ODR_155_HZ: {
            ...
            uint8_t a = device->write_reg(LIS3MDL_REGISTER::CTRL_REG1, data); // sets X and Y axes

            // now for Z axis
            byte ctrl4 = device->read_reg(LIS3MDL_REGISTER::CTRL_REG4);
            ctrl4 &= 0b10011111; // clear OM bits
            ctrl4 |= 0b01100000; // set OM bits to 11 for ultra-high performance mode
            uint8_t b = device->write_reg(LIS3MDL_REGISTER::CTRL_REG4, ctrl4);
...
uint8_t LIS3MDL::set_full_scale(LIS3MDL_FULL_SCALE full_scale) {
    ...
    byte data = device->read_reg(LIS3MDL_REGISTER::CTRL_REG2);
    ...
    return device->write_reg(LIS3MDL_REGISTER::CTRL_REG2, data);
}

uint8_t LIS3MDL::set_operating_mode(LIS3MDL_OPERATING_MODE mode) {
    byte data = device->read_reg(LIS3MDL_REGISTER::CTRL_REG3);
    data &= 0b11111100; // clear MD bits
    data |= mode; // set MD bits
    return device->write_reg(LIS3MDL_REGISTER::CTRL_REG3, data);
}
